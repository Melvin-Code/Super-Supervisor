...
function create() {
  ...
  let gameOverText = this.add.text(
    ...
    "GAME OVER",
    { fontSize: "40px", fill: "#fff", backgroundColor: "#000" }
  );
  ...
  setInterval(() => {
    if (enemyArr.length <= 3) {
      ...
      enemy = this.physics.add.sprite(x, 200, "enemy").setImmovable(true);
...
function update(time, delta) {
  ...
  enemyArr.forEach(d => {
    if (d.body.velocity.x < 0) {
      d.anims.play("walkEnemy", true); // walk left
      d.flipX = true;
    } else if (d.body.velocity.x > 0) {
      d.anims.play("walkEnemy", true);
      d.flipX = false;
    } else {
      console.log("no");
    }
